...
#define CACHE_CRCR_SE_Msk                      (uint32_t)(0x1UL << CACHE_CRCR_SE_Pos)               /*!< CACHE CRCR: SE Mask */
...
/**
  \brief   Enable secure cacheable range
  \details Enable secure cacheable range
  \param [in]  index      cache scope index (0, 1, 2, 3).
  */
__STATIC_INLINE void csi_cache_range_enable_security (uint32_t index)
{
    CACHE->CRCR[index] |= CACHE_CRCR_SE_Msk;
}

/**
  \brief   Disable secure cacheable range
  \details Disable secure cacheable range
  \param [in]  index      cache scope index (0, 1, 2, 3).
  */
__STATIC_INLINE void csi_cache_range_disable_security (uint32_t index)
{
    CACHE->CRCR[index] &= ~(CACHE_CRCR_SE_Msk);
}
