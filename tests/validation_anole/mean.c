...
{
    init_testsuite("Testing function of mean(anole).\n");
    ...
    float *src_in   = (float *)(buffer + 6 + axis_count);
    ...
    float *ref      = (float *)(buffer + 6 + axis_count + in_size);
    ...
    /*
        support Multi-axis (params->axis_count > 1)  if dim_count = 4  [ axis = -1 ] --> [ axis = 3 ]
        can not do reduce on batch and channel axis.
        if axis = batch  or  axis = channel:
        WARNING: (NNA_INTERNAL_FUNC) Unsupported input data format for: params [float 32]
        WARNING: (NNA_INTERNAL_FUNC) Unsupported output data format for: params [float 32]
    */
    if (csi_mean_init(input, output, &params) != CSINN_TRUE) {
        printf("mean init fail.\n\t");
        return -1;
    }
