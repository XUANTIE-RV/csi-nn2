...
{
    init_testsuite("Testing function of ropooling(anole).\n");
    ...
    struct csi_tensor *spatial_scale  = csi_alloc_tensor(sess);
    struct roi_pool_params params;
    float *spatial = (float *)(buffer + 11);
    params.spatial_scale = *(float *)(buffer + 11);

    printf("%f", *spatial);

    spatial_scale->qinfo = get_quant_info(spatial, 1);
    params.spatial_scale_multiplier = spatial_scale->qinfo->multiplier; 
    params.spatial_scale_shift = spatial_scale->qinfo->shift; 
    ...
    input1->dim[1] = buffer[4];
    input1->dim[2] = buffer[5];
    input1->dim[3] = buffer[6];
    input1->dim_count = 4;
    in1_size = input1->dim[0] * input1->dim[1] * input1->dim[2] * input1->dim[3];
  
    input1->name = "input1";
    float *src1_in  = (float *)(buffer + 12 + in0_size);
    ...
    output->dim[2] = buffer[7];
    output->dim[3] = buffer[8];
    output->dim_count = 4;
    out_size = output->dim[0] * output->dim[1] * output->dim[2] * output->dim[3];
    float *ref      = (float *)(buffer + 12 + in0_size + in1_size);
    ...
    params.pooled_size_h = buffer[9];
    params.pooled_size_w = buffer[10];
    ...
    /*
        software layer, unsupport broadcast??
    */
    if (csi_roipool_init(input0, input1, output, &params) != CSINN_TRUE) {
        printf("roipool init fail.\n\t");
        return -1;
    }
