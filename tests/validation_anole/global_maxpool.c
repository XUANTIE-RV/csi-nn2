...
{
    init_testsuite("Testing function of global_maxpool(anole).\n");
    ...
    if (csi_global_maxpool_init(input, output, &params) != CSINN_TRUE) {
        printf("global_maxpool init fail.\n\t");
        return -1;
    }
    ...
    csi_global_maxpool(input, output, &params);
