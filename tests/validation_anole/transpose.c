...
{
    init_testsuite("Testing function of transpose(anole).\n");
    ...
    float *src_in   = (float *)(buffer + 1 + 3 * input->dim_count);
    ...
    float *ref      = (float *)(buffer + 1 + 3 * input->dim_count + in_size);
    ...
    /*
        when params.permute = [ 0, 1, 2, ... input->dim_count-1 ] convert to hardware layer
    */
    if (csi_transpose_init(input, output, &params) != CSINN_TRUE) {
        printf("transpose init fail.\n\t");
        return -1;
    }
