...
{
    init_testsuite("Testing function of resize(anole).\n");
    ...
    params.resize_mode = CSINN_RESIZE_BILINEAR; // CSINN_RESIZE_NEAREST_NEIGHBOR
    params.align_corners = buffer[6];

    /*
    CSINN_RESIZE_BILINEAR:
    Unsupported non-integer upsampling rescale on width and height factor
    when align_corners = True:
        (out_h - 1) % (in_h - 1) == 0 && (out_w - 1) % (in_w - 1) == 0  for upsampling
        (in_h - 1) % (out_h - 1) == 0 && (in_w - 1) % (out_w - 1) == 0  for downsampling
    when align_corners = False:
        out_h % in_h == 0 && out_w % in_w == 0  for upsampling
        in_h % out_h == 0 && in_w % out_w == 0  for downsampling


    CSINN_RESIZE_NEAREST_NEIGHBOR:
    Unsupported non-integer upsampling rescale on width and height factor
    when align_corners = True:
        (out_h - 1) % (in_h - 1) == 0 && (out_w - 1) % (in_w - 1) == 0  for upsampling (wrong)???
        (in_h - 1) % (out_h - 1) == 0 && (in_w - 1) % (out_w - 1) == 0  for downsampling
    when align_corners = False:
        out_h % in_h == 0 && out_w % in_w == 0  for upsampling
        in_h % out_h == 0 && in_w % out_w == 0  for downsampling
