...
{
    init_testsuite("Testing function of prelu(anole).\n");
    ...
    uint8_t *alpha_tmp = malloc(buffer[1] * sizeof(char));

    alpha->qinfo = get_quant_info(alpha_in, alpha_size);

    for(int i = 0; i < alpha_size; i++) {
        alpha_tmp[i] = csi_ref_quantize_f32_to_u8(alpha_in[i], alpha->qinfo);
    }
    ...
    float *ref      = (float *)(buffer + 4 + alpha_size + in_size);
