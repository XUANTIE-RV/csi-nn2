...
void test_i8_sym(struct csi_tensor *input0, struct csi_tensor *input1,
                 struct csi_tensor *output, struct diso_params *params, float difference)
{
    printf("test mul i8 sym\n");
    ...
    struct csi_tensor *real_input0 = convert_f32_input(input0, CSINN_DTYPE_INT8, sess);
    struct csi_tensor *real_input1 = convert_f32_input(input1, CSINN_DTYPE_INT8, sess);
    op_test_run(qinput0, qinput1, qoutput, params, sess, real_input0, real_input1, output->data, difference);
}

void test_i16_sym(struct csi_tensor *input0, struct csi_tensor *input1,
                 struct csi_tensor *output, struct diso_params *params, float difference)
{
    printf("test mul i16 sym\n");
    struct csi_session *sess = csi_alloc_session();
    sess->base_api = CSINN_LIGHT;
    ...
    struct csi_tensor *qinput1 = convert_f32_input(input1, test_dtype, sess);
    struct csi_tensor *qoutput = convert_f32_input(output, test_dtype, sess);
    struct csi_tensor *real_input0 = convert_f32_input(input0, CSINN_DTYPE_INT16, sess);
    struct csi_tensor *real_input1 = convert_f32_input(input1, CSINN_DTYPE_INT16, sess);
    op_test_run(qinput0, qinput1, qoutput, params, sess, real_input0, real_input1, output->data, difference);
}

void test_i8_asym(struct csi_tensor *input0, struct csi_tensor *input1, struct csi_tensor *output,
                  struct diso_params *params, float difference)
{
    printf("test mul i8 asym\n");
    struct csi_session *sess = csi_alloc_session();
    sess->base_api = CSINN_LIGHT;
