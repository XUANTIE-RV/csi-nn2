...
void test_i8_sym(struct csi_tensor *input, struct csi_tensor *output, struct relu_params *params,
                 float difference)
{
    printf("test relu1 i8 sym\n");
...
void test_i16_sym(struct csi_tensor *input, struct csi_tensor *output, struct relu_params *params,
                 float difference)
{
    printf("test relu1 i16 sym\n");
    struct csi_session *sess = csi_alloc_session();
    sess->base_api = CSINN_LIGHT;
...
void test_i8_asym(struct csi_tensor *input, struct csi_tensor *output, struct relu_params *params,
                 float difference)
{
    printf("test relu1 i8 asym\n");
    struct csi_session *sess = csi_alloc_session();
    sess->base_api = CSINN_LIGHT;
    ...
    struct csi_tensor *qoutput = convert_f32_input(input, test_dtype, sess);
    struct csi_tensor *real_input = convert_f32_input(input, test_dtype, sess);

    op_test_run(qinput, qoutput, params, sess, real_input, output->data, difference);
}

void test_u8_asym(struct csi_tensor *input, struct csi_tensor *output, struct relu_params *params,
                 float difference)
{
    printf("test relu1 u8 asym\n");
    struct csi_session *sess = csi_alloc_session();
    sess->base_api = CSINN_LIGHT;
