...
int main(int argc, char** argv)
{
    ...
    if(flag) {
        input1->dim[0] = input0->dim[0];    // batch
        input1->dim[1] = input0->dim[1];    // channel
        input1->dim[2] = 1;
        input1->dim[3] = 1;
        input1->dim_count = 4;
    ...
    output->dim[0] = csi_ref_max_internal_s32(input0->dim[0], input1->dim[0]);  // in fact, ouput->dim[i] are always equal to input0->dim[i]
    output->dim[1] = csi_ref_max_internal_s32(input0->dim[1], input1->dim[1]);
    output->dim[2] = csi_ref_max_internal_s32(input0->dim[2], input1->dim[2]);
    output->dim[3] = csi_ref_max_internal_s32(input0->dim[3], input1->dim[3]);
    output->dim_count = 4;
    ...
    /*
        input tensor has unsupported batch broadcast
    */
