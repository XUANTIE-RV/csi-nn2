...
int main(int argc, char** argv)
{
    ...
    struct csi_tensor *alpha  = csi_alloc_tensor(sess);
    ...
    get_quant_info(alpha);

    void *alpha_tmp = malloc(alpha_size * sizeof(char));
    for(int i = 0; i < alpha_size; i++) {
        if (sess->base_dtype == CSINN_DTYPE_UINT8) {
            *((uint8_t *)alpha_tmp + i) = csi_ref_quantize_f32_to_u8(alpha_data[i], alpha->qinfo);
        } else if (sess->base_dtype == CSINN_DTYPE_INT8) {
            *((int8_t *)alpha_tmp + i) = csi_ref_quantize_f32_to_i8(alpha_data[i], alpha->qinfo);
        }
    }
    if (sess->base_dtype == CSINN_DTYPE_UINT8 || sess->base_dtype == CSINN_DTYPE_INT8) {
        alpha->data = alpha_tmp;
    }
    ...
    /* light: tensor B has unsupported batch broadcast  -->  batch = 1 or alpha->dim_count = 2 */
    if (csi_prelu_init(input, alpha, output, &params) != CSINN_TRUE) {
        printf("prelu init fail.\n\t");
        return -1;
    }
