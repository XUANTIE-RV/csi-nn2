...
struct csi_session *create_session_base(int input_num, int output_num)
{
    ...
    csi_set_input_number(input_num, sess);
    csi_set_output_number(output_num, sess);

    return sess;
}

struct csi_tensor *create_tensor_base(char *name, struct csi_session *sess, int *shape,
                                      int shape_len)
{
    ...
    tensor->qinfo = (struct csi_quant_info *)malloc(sizeof(struct csi_quant_info));
...
struct csi_ref_graph *convert_graph2subgraph(struct csi_ref_graph *ograph)
{
...
void test_model1()
{
    ...
    struct csi_session *sess = create_session_base(1, 1);

    int input_shape[] = {1, 3, 32, 32};
    struct csi_tensor *input = create_tensor_base("input", sess, input_shape, 4);
    struct relu_params *params_1 = csi_alloc_params(sizeof(struct relu_params), sess);
    params_1->base.layout = CSINN_LAYOUT_NCHW;
    params_1->base.name = "params_1";
    params_1->base.api = CSINN_LIGHT;
    struct csi_tensor *relu1_out = create_tensor_base("relu1_out", sess, input_shape, 4);
    csi_relu_init(input, relu1_out, params_1);
    ...
    params_2->base.api = CSINN_LIGHT;
    struct csi_tensor *relu2_out = create_tensor_base("relu2_out", sess, input_shape, 4);
    csi_relu_init(relu1_out, relu2_out, params_2);

    struct softmax_params *params_3 = csi_alloc_params(sizeof(struct softmax_params), sess);
    params_3->axis = 1;
    params_3->base.layout = CSINN_LAYOUT_NCHW;
    params_3->base.name = "params_3";
    struct csi_tensor *softmax_out = create_tensor_base("softmax_out", sess, input_shape, 4);
    csi_softmax_init(relu2_out, softmax_out, params_3);
    ...
    params_4->base.api = CSINN_LIGHT;
    struct csi_tensor *relu3_out = create_tensor_base("relu3_out", sess, input_shape, 4);
    csi_relu_init(softmax_out, relu3_out, params_4);

    csi_set_tensor_entry(input, sess);
    csi_set_input(0, input, sess);

    csi_relu(input, relu1_out, params_1);
    csi_relu(relu1_out, relu2_out, params_2);
    csi_softmax(relu2_out, softmax_out, params_3);
    csi_relu(softmax_out, relu3_out, params_4);

    csi_set_output(0, relu3_out, sess);

    struct csi_ref_graph *graph = csi_gref_get_graph(sess);
    struct csi_ref_graph *ggraph = convert_graph2subgraph(graph);
...
void test_model2()
{
    ...
    params_3->base.name = "params_3";
    struct csi_tensor *softmax_out = create_tensor_base("softmax_out", sess, input_shape, 4);
    csi_softmax_init(relu1_out, softmax_out, params_3);

    struct diso_params *params_4 = csi_alloc_params(sizeof(struct diso_params), sess);
    params_4->base.name = "params_4";
    params_4->base.api = CSINN_LIGHT;
    struct csi_tensor *add_out = create_tensor_base("add_out", sess, input_shape, 4);
    csi_add_init(relu2_out, softmax_out, add_out, params_4);
    ...
    csi_relu(relu1_out, relu2_out, params_2);
    csi_softmax(relu2_out, softmax_out, params_3);
    csi_add(relu2_out, softmax_out, add_out, params_4);

    csi_set_output(0, add_out, sess);
