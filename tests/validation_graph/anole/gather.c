...
{
    init_testsuite("Testing function of gather (anole).\n");

    int *buffer = read_input_data_f32(argv[1]);
    int axis = buffer[0];
    int input_dim_count = buffer[1];
    int indices_dim_count = buffer[2 + input_dim_count];
    ...
    input->name = "input";
    float *input_data   = (float *)(buffer + 3 + input_dim_count + indices_dim_count);
    ...
    for (int i = 0; i < indices->dim_count; i++) {
        indices->dim[i] = buffer[3 + input_dim_count + i];
        indices_size *= indices->dim[i];
    }
    indices->name = "indices";
    indices->dtype = CSINN_DTYPE_INT32;
    int32_t *indices_data   = (int32_t *)(buffer + 3 + input_dim_count + indices_dim_count + in_size);
