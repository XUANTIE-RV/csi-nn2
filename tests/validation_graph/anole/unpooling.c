...
{
    init_testsuite("Testing function of unpooling(anole).\n");

    int *buffer = read_input_data_f32(argv[1]);

    struct csi_tensor *reference = csi_alloc_tensor(NULL);
    int in_size = 0, out_size = 0, mask_size=0;
    ...
    /* mask tensor configuration */
    struct csi_tensor *mask  = csi_alloc_tensor(sess);
    mask->dim[0] = buffer[0];          // batch
    mask->dim[1] = buffer[1];          // channel
    mask->dim[2] = buffer[2];          // height
    mask->dim[3] = buffer[3];          // width
    mask->dim_count = 4;
    mask_size = input->dim[0] * input->dim[1] * input->dim[2] * input->dim[3];
    int32_t *mask_in = (int32_t *)(buffer + 14 + in_size);
    mask->name = "mask";
    ...
    reference->data = (float *)(buffer + 14 + in_size + mask_size);
    ...
    params.pad_out_height = 0;
    params.pad_out_width = 0;
    params.scale_height = buffer[6];
    params.scale_width = buffer[7];


    if (csi_unpooling_init(input, mask, output, &params) != CSINN_TRUE) {
        printf("unpooling init fail.\n\t");
        return -1;
    }

    csi_set_tensor_entry(input, sess);
    csi_set_tensor_entry(mask, sess);
    csi_set_input(0, input, sess);
    csi_set_input(1, mask, sess);

    csi_unpooling(input, mask, output, &params);
    ...
    struct csi_tensor *mask_tensor = csi_alloc_tensor(NULL);
    input_tensor->data = src_tmp;
    mask_tensor->data = mask_in;
    csi_update_input(0, input_tensor, sess);
    csi_update_input(1, mask_tensor, sess);
