...
{
    init_testsuite("Testing function of greater_equal(anole).\n");
    ...
    output->dim[0]  = input0->dim[0];    // batch
    output->dim[1]  = input0->dim[1];    // channel
    output->dim[2]  = input0->dim[2];    // height
    output->dim[3]  = input0->dim[3];    // width
    output->dim_count = 4;
    ...
    if (csi_greater_equal_init(input0, input1, output, &params) != CSINN_TRUE) {
        printf("greater_equal init fail.\n\t");
        return -1;
    }
    ...
    csi_greater_equal(input0, input1, output, &params);
