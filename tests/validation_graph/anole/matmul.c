...
{
    init_testsuite("Testing function of matmul(anole).\n");

    int *buffer = read_input_data_f32(argv[1]);
    int in_out_dims = buffer[2];
    ...
    for (int i = 0; i < input0->dim_count; ++i) {
        ...
        printf("input0_dim %d %d\n", i, input0->dim[i]);
        in0_size *= input0->dim[i];
    }
    input0->name = "input";
    float *input0_data = (float *)(buffer + 3 + 3 * in_out_dims);
    ...
    for (int i = 0; i < input1->dim_count; ++i) {
        input1->dim[i] = buffer[3 + i + in_out_dims];
        printf("input1_dim %d %d\n", i, input1->dim[i]);
        in1_size *= input1->dim[i];
    }
    input1->name = "input1";
    float *input1_data  = (float *)(buffer + 3 + 3 * in_out_dims + in0_size);
    ...
    for (int i = 0; i < output->dim_count; ++i) {
        output->dim[i] = buffer[3 + i + 2 * in_out_dims];
        printf("output_dim %d %d\n", i, output->dim[i]);
        out_size *= output->dim[i];
    }
    reference->data = (float *)(buffer + 3 + 3 * in_out_dims + in0_size + in1_size);
