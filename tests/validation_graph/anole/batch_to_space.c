...
{
    init_testsuite("Testing function of batch_to_space(graph).\n");

    int *buffer = read_input_data_f32(argv[1]);
    int block_size = buffer[4];
    int crop_top = buffer[5];
    int crop_bottom = buffer[6];
    int crop_left = buffer[7];
    int crop_right = buffer[8];
    ...
    output->dim[2]  = input->dim[2] * block_size - crop_top - crop_bottom;    // height
    output->dim[3]  = input->dim[3] * block_size - crop_left - crop_right;    // width
    output->dim_count = 4;
    ...
    if (csi_batch_to_space_init(input, output, &params) != CSINN_TRUE) {
        printf("batch_to_space init fail.\n");
        return -1;
    }
