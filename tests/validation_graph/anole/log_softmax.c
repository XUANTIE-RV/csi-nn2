...
{
    init_testsuite("Testing function of log_softmax(anole).\n");
    ...
    input->name = "input";
    float *input_data   = (float *)(buffer + 2 + input->dim_count);
    ...
    /*
    anole: only support 2D input tensor ? assert(input->dim_count=2)  assert(axis=1)
    */
    if (csi_log_softmax_init(input, output, &params) != CSINN_TRUE) {
        printf("log_softmax init fail.\n\t");
        return -1;
    }
