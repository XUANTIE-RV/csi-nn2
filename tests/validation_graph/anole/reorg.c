...
{
    init_testsuite("Testing function of reorg(anole).\n");
    ...
    output->dim[1]  = input->dim[1] * stride * stride;
    output->dim[2]  = input->dim[2] / stride;
    output->dim[3]  = input->dim[3] / stride;
    output->dim_count = 4;
    ...
    if (csi_reorg_init(input, output, &params) != CSINN_TRUE) {
        printf("reorg init fail.\n\t");
        return -1;
    }
