...
{
    init_testsuite("Testing function of depthwise convolution1d(layer).\n");
    ...
    weight_size = kernel->dim[0] * kernel->dim[1] * kernel->dim[2];
    params->base.api = CSINN_API;

    input->data     = (float *)(buffer + 10);
    kernel->data    = (float *)(buffer + 10 + in_size);
    bias->data = (float *)(buffer + 10 + in_size + weight_size);
    reference->data = (float *)(buffer + 10 + in_size + weight_size + output->dim[1]);
