...
int main(int argc, char** argv)
{
    ...
    input->dim_count = 4;
    output->dim_count = shape_len;
    for(int i = 0; i < shape_len; i++) {
        params.shape[i] = buffer[5 + i];
    ...
    input->data      = (float *)(buffer + 5 + shape_len);
    reference->data  = (float *)(buffer + 5 + shape_len + in_size);
