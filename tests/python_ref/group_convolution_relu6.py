...
def group_convolution_f32():
    ...
    out_channel = int(np.random.randint(32, high=64, size=1))
    ...
    #src_in = np.arange(70).reshape(1, 2, 7, 5)
    ...
    t_src_out1 = fn.relu6(t_src_out1).numpy()
    ...
    src_in_sp  = np.split(src_in_nhwc, group, axis=3)

    src_out_sp = np.split(out_nhwc, group, axis=3)
    ...
    src_in_nhwc = src_in_nhwc.flatten()
    weight_1  = weight_nhwc.flatten()
    out_nhwc = out_nhwc.flatten()

    total_size = (len(src_in_nhwc) + len(out_nhwc)) + len(weight_1) + len(bias) + 18
    ...
    with open("group_convolution_relu6_data_f32.bin", "wb") as fp:
