...
def reduce_logsumexp_f32():
    para = []
    # init the input data and parameters
    batch      = int(np.random.randint(1, high=8, size=1))
    ...
    if reduce_dim==-1:
        out_calcu = tf.reduce_logsumexp(src_in)
    else:
        out_calcu = tf.reduce_logsumexp(src_in, axis=reduce_dim)
    ...
    with open("reduce_logsumexp_data_f32.bin", "wb") as fp:
