...
def unsorted_segment_mean_f32():
    para = []
    # init the input data and parameters
    batch      = int(np.random.randint(10, high=32, size=1))
    in_size_x  = int(np.random.randint(1, high=32, size=1))
    in_size_y  = int(np.random.randint(1, high=32, size=1))
    ...
    num_segments = np.sort(segment_ids)[batch-1] + 1

    out_calcu = tf.unsorted_segment_mean(tf.convert_to_tensor(src_in), segment_ids, num_segments)
    ...
    with open("unsorted_segment_mean_data_f32.bin", "wb") as fp:
