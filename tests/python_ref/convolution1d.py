...
def convolution_f32():
    ...
    #permute ncw to nwc
    src_in_nhwc = np.transpose(src_in, [0, 2, 1])
    weight_nhwc = np.transpose(weight, [0, 2, 1])
    out_nhwc    = np.transpose(t_src_out1, [0, 2, 1])

    out_size_x = np.shape(out_nhwc)[1]
    ...
    with open("convolution1d_data_f32.bin", "wb") as fp:
