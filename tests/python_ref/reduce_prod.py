...
def reduce_prod_f32():
    ...
    in_size_x  = int(np.random.randint(8, high=128, size=1))
    ...
    if reduce_dim==-1:
        out_calcu = tf.reduce_prod(src_in)
    else:
        out_calcu = tf.reduce_prod(src_in, axis=reduce_dim)
    ...
    with open("reduce_prod_data_f32.bin", "wb") as fp:
