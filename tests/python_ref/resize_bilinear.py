...
def resize_bilinear_f32():
    ...
    out_size_x = int(np.random.randint(17, high=18, size=1))
    out_size_y = int(np.random.randint(18, high=19, size=1))
    ...
    out_calcu = tf.image.resize_bilinear(tf.convert_to_tensor(src_in), [out_size_y, out_size_x], align_corners=alco)
    ...
    src_in_nhwc = src_in
    src_out_nhwc = src_out

    src_in_1  = src_in_nhwc.flatten()
    src_out_1 = src_out_nhwc.flatten()
    ...
    with open("resize_bilinear_data_f32.bin", "wb") as fp:
