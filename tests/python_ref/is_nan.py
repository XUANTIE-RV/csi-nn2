...
def is_nan_f32():
    ...
    for i in range(0, in_dim):
        in_shape.append(int(np.random.randint(128, high=512, size=1)))
    
    choice = int(np.random.randint(10, high=20, size=1))

    zero_point  = int(np.random.randint(-600, high=600, size=1))
    std         = int(np.random.randint(1, high=20, size=1))

    src_in = np.random.normal(zero_point, std, in_shape)
    src_in.ravel()[np.random.choice(src_in.size, choice, replace=False)] = np.nan
    src_in = src_in.astype(np.float32)
    

    out_calcu = tf.is_nan(src_in)
    ...
    with open("is_nan_data_f32.bin", "wb") as fp:
        data = struct.pack(('%di' % len(para)), *para)
        fp.write(data)
        data = struct.pack(('%df' % len(src_in_1)), *src_in_1)
        fp.write(data)
        data = struct.pack(('%d?' % len(src_out_1)), *src_out_1)
        fp.write(data)
        fp.close()
