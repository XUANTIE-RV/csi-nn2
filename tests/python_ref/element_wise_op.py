...
def element_wise_op_f32():
    para = []
    # init the input data and parameters
    batch       = int(np.random.randint(1, high=10, size=1))
    in_size_x   = int(np.random.randint(128, high=1024, size=1))
    in_size_y   = int(np.random.randint(128, high=1024, size=1))
    in_channel  = int(np.random.randint(1, high=128, size=1))
    ...
    with open("element_wise.bin", "wb") as fp:
