...
def batch_to_space_nd_f32():
    ...
    for i in range(0, spatial_shape_cnt):
        ...
        block_size = int(np.random.randint(1, high=2, size=1))   # block_size of each spatial dim
        block_shape.append(block_size)
        batch *= block_size

        crop_start = int(np.random.randint(0, high=1, size=1))
        crop_end   = int(np.random.randint(0, high=1, size=1))
    ...
    for i in range(0, 1 + spatial_shape_cnt + remain_shape_cnt):
    ...
    with open("batch_to_space_nd_data_f32.bin", "wb") as fp:
