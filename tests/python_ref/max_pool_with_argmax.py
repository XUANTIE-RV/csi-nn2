...
def max_pool_with_argmax_f32():
    ...
    out_nhwc    = np.transpose(t_src_out[0], [3, 1, 0, 2])
    out_indices = np.transpose(t_src_out[1], [3, 1, 0, 2])
    #out_indices = t_src_out[1]
    #print(len(out_indices))

    src_in_1      = src_in_nhwc.flatten()
    src_out_1     = out_nhwc.flatten()
    out_indices_1 = out_indices.flatten()
    #print(batch*out_y*out_x*in_channel)

    total_size = (len(src_in_1) + len(src_out_1)) + len(out_indices_1) + 12
    ...
    with open("max_pool_with_argmax_f32_data.bin", "wb") as fp:
        ...
        fp.write(data)
        data = struct.pack(('%di' % len(out_indices_1)), *out_indices_1)
        fp.write(data)
        fp.close()
