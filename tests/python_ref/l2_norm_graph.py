...
def l2_normalization_f32():
    ...
    for i in range(0, dim_count):
        in_size = int(np.random.randint(4, high=16, size=1))
        dim.append(in_size)
    ...
    out_calcu = tf.nn.l2_normalize(tf.convert_to_tensor(src_in), epsilon=epsi, axis=(1,2,3))
    ...
    with open("l2_norm_graph_data_f32.bin", "wb") as fp:
