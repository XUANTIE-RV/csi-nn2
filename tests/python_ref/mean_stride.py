...
def mean_f32():
    ...
    tensor=tf.constant(src_in1)
    mean_a_nokeepdims = tf.keras.backend.mean(tensor, axis=axis, keepdims=False)

    init = tf.global_variables_initializer()
    with tf.Session() as sess:
        sess.run(init)
        result = sess.run(mean_a_nokeepdims)
    ...
    with open("mean_stride_data_f32.bin", "wb") as fp:
