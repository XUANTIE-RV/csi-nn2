...
def cumprod_f32():
    ...
    src_in1 = np.random.uniform(1, 2, (batch, in_size_x, in_size_y, in_channel))
    src_in1 = src_in1.astype(np.float32)

    # src_out = np.cumsum(src_in1, axis = dim)
    out_calcu = tf.cumprod(src_in1, axis = dim, exclusive = True if excl else False)
    ...
    with open("cumprod_data_f32.bin", "wb") as fp:
