...
def select_f32():
    ...
    high_val  =  np.float32(np.random.randint(300, high=40000, size=1))
    condlist  = (src_in1 < high_val)

    src_out   = np.select(condlist, src_in1, src_in2)

    src_in_1   = src_in1.flatten()
    src_in_2   = src_in2.flatten()
    condlist_1 = condlist.flatten()
    src_out_1  = src_out.flatten()

    total_size = 3 *(len(src_in_1) + len(src_out_1)) + 4
    ...
    with open("select_data_f32.bin", "wb") as fp:
        ...
        fp.write(data)
        data = struct.pack(('%df' % len(condlist_1)), *condlist_1)
        fp.write(data)
