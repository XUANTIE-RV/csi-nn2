...
static int csi_logical_or_f32(struct csi_tensor *input0,
...
static int csi_logical_or_u8(struct csi_tensor *input0,
                struct csi_tensor *input1,
                struct csi_tensor *output,
                 struct diso_params *params)
{
                 ...
                 struct diso_params *params)
{
    if (input0->dtype == CSINN_DTYPE_UINT8) {
        params->bc = csi_logical_or_u8;
    } else if (input0->dtype == CSINN_DTYPE_FLOAT32) {
        params->bc = csi_logical_or_f32;
    } else {
