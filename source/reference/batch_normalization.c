...
static int csi_batch_normalization_nhwc_f32(struct csi_tensor *input,
...
static int csi_batch_normalization_nhwc_u8(struct csi_tensor *input,
                                        struct csi_tensor *mean,
                                        struct csi_tensor *variance,
                                        struct csi_tensor *gamma,
                                        struct csi_tensor *beta,
                                        ...
                                        struct bn_params *params)
{
    ...
    for (int b = 0; b < batches; ++b) {
        for (int c = 0; c < input->dim[dims_count - 1]; ++c) {
            float intput_val = csi_dequantize_f32(input_data[b * batch_offset + c], input->offset,
                                            input->multiplier, input->shift);
            float mean_val   = csi_dequantize_f32(mean_data[c], mean->offset, mean->multiplier,
                                            mean->shift);
            float var_val    = csi_dequantize_f32(var_data[c], variance->offset, variance->multiplier,
                                            variance->shift);
            float beta_val   = csi_dequantize_f32(beta_data[c], beta->offset, beta->multiplier,
                                            beta->shift);
            float result = 1/sqrt(var_val + params->epsilon);
            result *= (intput_val - mean_val);
            if (gamma != NULL) {
                uint8_t *gamma_data  = gamma->data;
                result *=  csi_dequantize_f32(gamma_data[c], gamma->offset, gamma->multiplier,
                                            gamma->shift);
            }
            result += beta_val;
            output_data[b * batch_offset + c] = csi_quantize_f32(result, output->offset,
                                 ...
                                 struct bn_params *params)
{
    if (input->dtype == CSINN_DTYPE_UINT8) {
        if (params->layout == CSINN_NHWC) {
            params->bc = csi_batch_normalization_nhwc_u8;
        } else if (params->layout == CSINN_NCHW) {
            return CSINN_UNSUPPORT_DTYPE;
        }
    } else if (input->dtype == CSINN_DTYPE_FLOAT32) {
        if (params->layout == CSINN_NHWC) {
            params->bc = csi_batch_normalization_nhwc_f32;
        } else if (params->layout == CSINN_NCHW) {
            return CSINN_UNSUPPORT_DTYPE;
        }
    } else {
