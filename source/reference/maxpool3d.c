            ...
            for(int out_d=0; out_d<out_depth; ++out_d) {
                for(int out_h=0; out_h<out_height; ++out_h) {
                    for(int out_w=0; out_w<out_width; ++out_w) {
                        ...
                        const int filter_d_end = csi_ref_min_internal_s32(params->filter_depth, in_depth - in_d_origin);
                        const int filter_h_begin = csi_ref_max_internal_s32(0, -in_h_origin);
                        const int filter_h_end = csi_ref_min_internal_s32(params->filter_height, in_height - in_h_origin);
                        const int filter_w_begin = csi_ref_max_internal_s32(0, -in_w_origin);
                        const int filter_w_end = csi_ref_min_internal_s32(params->filter_width, in_width - in_w_origin);
                        ...
                        for(int filter_d=filter_d_begin; filter_d<filter_d_end; ++filter_d) {
                            for(int filter_h=filter_h_begin; filter_h<filter_h_end; ++filter_h) {
                                for(int filter_w=filter_w_begin; filter_w<filter_w_end; ++filter_w) {
                                    ...
                                    max = fmax(max, input_data[csi_ref_get_index_5(input->dim, in_ch, out_ch, in_d, in_h, in_w)]);
                                    filter_cnt++;
                                }
                            }
                        }
                        ...
                        output_data[csi_ref_get_index_5(output->dim, in_ch, out_ch, out_d, out_h, out_w)] = max;
                    }
                }
            }
        }
