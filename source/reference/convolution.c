...
static float uint8_to_float(uint8_t i, struct csi_tensor *t)
{
    return ((float)i - t->zero_point) * t->scale;
}

static uint8_t float_to_uint8(float i, struct csi_tensor *t)
{
                              ...
                              struct conv2d_params *params)
{
    ...
    kernel = csi_nchw_to_nhwc_u8(o_kernel);
...
static int64_t conv_out_u8(int64_t res,
                           struct csi_tensor *input,
                           struct csi_tensor *output,
                           struct csi_tensor *kernel)
{
...
static int64_t conv_relu6_out_u8(int64_t res,
                                 struct csi_tensor *input,
                                 struct csi_tensor *output,
                                 struct csi_tensor *kernel)
{
...
static int csi_conv2d_relu_nchw_u8(struct csi_tensor *o_input,
                                   struct csi_tensor *o_output,
                                   struct csi_tensor *o_kernel,
                                   struct csi_tensor *o_bias,
                                   struct conv2d_params *params)
        ...
        for (int32_t out_y = 0; out_y < output_height; ++out_y) {
            for (int32_t out_x = 0; out_x < output_width; ++out_x) {
                for (int32_t out_channel = 0; out_channel < output_depth; ++out_channel) {
                    ...
                    acc = conv_out_u8(acc, input, output, kernel);
                    output_data[csi_get_index(output->dim, batch, out_y, out_x, out_channel)] = acc;
                }
            }
        }
...
static int csi_conv2d_relu6_nchw_u8(struct csi_tensor *o_input,
                                    struct csi_tensor *o_output,
                                    struct csi_tensor *o_kernel,
                                    struct csi_tensor *o_bias,
                                    struct conv2d_params *params)
...
{
#if 0
    csi_group_conv2d_u8(o_input, o_output, o_kernel, o_bias,
                        o_input->dim[1], stride_height, stride_width,
...
static int csi_depthwise_conv2d_relu_nchw_u8(struct csi_tensor *o_input,
                                             struct csi_tensor *o_output,
                                             struct csi_tensor *o_kernel,
                                             struct csi_tensor *o_bias,
                                             struct conv2d_params *params)
...
static int csi_depthwise_conv2d_relu6_nchw_u8(struct csi_tensor *o_input,
                                              struct csi_tensor *o_output,
                                              struct csi_tensor *o_kernel,
                                              struct csi_tensor *o_bias,
                                              struct conv2d_params *params)
...
static int csi_group_conv2d_relu_nchw_u8(struct csi_tensor *o_input,
                                         struct csi_tensor *o_output,
                                         struct csi_tensor *o_kernel,
                                         struct csi_tensor *o_bias,
                                         struct conv2d_params *params)
{
    ...
    for (int i = 0; i < params->group; i++) {
        ...
        kernel.data = kernel_data + i * kernel_size;
        bias.data = bias_data + i * o_output->dim[1] / params->group;
        csi_conv2d_relu_nchw_u8(&input, &output, &kernel, &bias, params);
    }
    return CSINN_TRUE;
}
    ...
    if (params->layout == CSINN_NCHW) {
        if (params->group == 1) {
            if (input->dtype == CSINN_DTYPE_UINT8) {
                params->bc = csi_conv2d_nchw_u8;
            } else {
                return CSINN_UNSUPPORT_DTYPE;
            }
        } else if (params->group == input->dim[1]) {
            if (input->dtype == CSINN_DTYPE_UINT8) {
                params->bc = csi_depthwise_conv2d_nchw_u8;
            } else {
                return CSINN_UNSUPPORT_DTYPE;
            }
        } else {
            if (input->dtype == CSINN_DTYPE_UINT8) {
                params->bc = csi_group_conv2d_nchw_u8;
            } else {
                return CSINN_UNSUPPORT_DTYPE;
            }
        }
        ...
        if (params->group == 1) {
            if (input->dtype == CSINN_DTYPE_UINT8) {
                params->bc = csi_conv2d_nhwc_u8;
            } else if (input->dtype == CSINN_DTYPE_FLOAT32){
                params->bc = csi_conv2d_nhwc_f32;
            }
        } else if (params->group == input->dim[3]) {
            if (input->dtype == CSINN_DTYPE_UINT8) {
                params->bc = csi_depthwise_conv2d_nhwc_u8;
            } else if (input->dtype == CSINN_DTYPE_FLOAT32) {
                params->bc = csi_depthwise_conv2d_nhwc_f32;
            }
        } else {
            if (input->dtype == CSINN_DTYPE_UINT8) {
                params->bc = csi_group_conv2d_nhwc_u8;
            } else if (input->dtype == CSINN_DTYPE_FLOAT32){
                params->bc = csi_group_conv2d_nhwc_f32;
            }
        }
    } else {
                         ...
                         struct conv2d_params *params)
{
    if (params->layout == CSINN_NCHW) {
        if (params->group == 1) {
            if (input->dtype == CSINN_DTYPE_UINT8) {
                params->bc = csi_conv2d_relu_nchw_u8;
            } else {
                return CSINN_UNSUPPORT_DTYPE;
            }
        } else if (params->group == output->dim[1]) {
            if (input->dtype == CSINN_DTYPE_UINT8) {
                params->bc = csi_depthwise_conv2d_relu_nchw_u8;
            } else {
                return CSINN_UNSUPPORT_DTYPE;
            }
        } else {
            if (input->dtype == CSINN_DTYPE_UINT8) {
                params->bc = csi_group_conv2d_relu_nchw_u8;
            } else {
                return CSINN_UNSUPPORT_DTYPE;
            }
        }
                          ...
                          struct conv2d_params *params)
{
    if (params->layout == CSINN_NCHW) {
        if (params->group == 1) {
            if (input->dtype == CSINN_DTYPE_UINT8) {
                params->bc = csi_conv2d_relu6_nchw_u8;
            } else {
                return CSINN_UNSUPPORT_DTYPE;
            }
        } else if (params->group == output->dim[1]) {
            if (input->dtype == CSINN_DTYPE_UINT8) {
                params->bc = csi_depthwise_conv2d_relu6_nchw_u8;
            } else {
                return CSINN_UNSUPPORT_DTYPE;
            }
        } else {
