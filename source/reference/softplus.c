...
static int csi_softplus_f32(struct csi_tensor *input,
                            ...
                            struct siso_params *params)
{
    ...
    for(int i = 0; i < size; i++) {
        output_data[i] = log(1+exp(input_data[i]));
    }
    return CSINN_TRUE;
}

static int csi_softplus_u8(struct csi_tensor *input,
                            struct csi_tensor *output,
                            struct siso_params *params)
{
    ...
    for(int i = 0; i < size; i++) {
        float input0_val = csi_dequantize_f32(input_data[i], input->offset, input->multiplier,
                                               input->shift);
        float res = log(1+exp(input0_val));
                      ...
                      struct siso_params *params)
{
    if(input->dtype == CSINN_DTYPE_UINT8) {
        params->bc = csi_softplus_u8;
    } else if (input->dtype == CSINN_DTYPE_FLOAT32) {
        params->bc = csi_softplus_f32;
    } else {
