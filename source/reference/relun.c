...
static int csi_relun_f32(struct csi_tensor *input,
...
static int csi_relun_u8(struct csi_tensor *input,
                         struct csi_tensor *output,
                         struct relu_params *params)
{
    ...
    float n = csi_dequantize_f32(1, 0, params->n_multiplier, params->n_shift);
    for (int i = 0; i < size; i++) {
                   ...
                   struct relu_params *params)
{
    if (input->dtype == CSINN_DTYPE_UINT8) {
        params->bc = csi_relun_u8;
    } else if (input->dtype == CSINN_DTYPE_FLOAT32) {
        params->bc = csi_relun_f32;
    } else {
