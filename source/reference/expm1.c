...
static int csi_expm1_f32(struct csi_tensor *input,
                       ...
                       struct siso_params *params)
{
    ...
    for (int i = 0; i < size; i++) {
        output_data[i] = exp(input_data[i]) - 1;
    }
    return CSINN_TRUE;
}

static int csi_expm1_u8(struct csi_tensor *input,
                      struct csi_tensor *output,
                      struct siso_params *params)
{
                 ...
                 struct siso_params *params)
{
    if (input->dtype == CSINN_DTYPE_UINT8) {
        params->bc = csi_expm1_u8;
    } else if (input->dtype == CSINN_DTYPE_FLOAT32) {
        params->bc = csi_expm1_f32;
    } else {
