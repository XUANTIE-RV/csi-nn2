...
static int csi_sum_stride_f32(struct csi_tensor *input,
...
static int csi_sum_stride_u8(struct csi_tensor *input,
                             struct csi_tensor *output,
                             struct reduce_params *params)
{
                    ...
                    struct reduce_params *params)
{
    if (params->n == 0 && params->m == 0) {
        ...
        if (input->dtype == CSINN_DTYPE_UINT8) {
            params->bc = csi_sum_stride_u8;
        } else if (input->dtype == CSINN_DTYPE_FLOAT32) {
            params->bc = csi_sum_stride_f32;
        } else {
