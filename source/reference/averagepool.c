...
int csi_ref_averagepool_nhwc_f32(struct csi_tensor *input,
...
static int csi_ref_averagepool_nchw_f32(struct csi_tensor *input,
                                        struct csi_tensor *output,
                                        struct pool_params *params)
{
    float *input_data = input->data;
                            ...
                            struct pool_params *params)
{
    if (params->base.layout == CSINN_LAYOUT_NCHW) {
        csi_ref_averagepool_nchw_f32(input, output, params);
    } else if (params->base.layout = CSINN_LAYOUT_NHWC) {
        csi_ref_averagepool_nhwc_f32(input, output, params);
    } else {
...
int csi_ref_averagepool_quant(struct csi_tensor *input,
                              ...
                              struct pool_params *params)
{
    return csi_ref_siso_callback_base(input, output, params, csi_ref_averagepool_f32);
}
