        ...
        for (int32_t out_y = 0; out_y < output_height; ++out_y) {
            for (int32_t out_x = 0; out_x < output_width; ++out_x) {
                for (int32_t out_channel = 0; out_channel < output_depth; ++out_channel) {
                    ...
                    acc = conv_out_i8(acc, input, output, kernel);
                    output_data[csi_get_index(output->dim, batch, out_y, out_x, out_channel)] = acc;
                }
            }
        }
...
int csi_group_conv2d_relu_u8(struct csi_tensor *o_input,
                             struct csi_tensor *o_output,
                             struct csi_tensor *o_kernel,
                             struct csi_tensor *o_bias,
                             struct conv2d_params *params)
{
    ...
    for (int i = 0; i < params->group; i++) {
        ...
        csi_conv2d_relu_u8(&input, &output, &kernel, &bias, &pparams);
    }
    return CSINN_TRUE;
}

int csi_group_conv2d_relu_i8(struct csi_tensor *o_input,
                             struct csi_tensor *o_output,
                             struct csi_tensor *o_kernel,
                             struct csi_tensor *o_bias,
                             struct conv2d_params *params)
{
    ...
    for (int i = 0; i < params->group; i++) {
        ...
        kernel.data = kernel_data + i * kernel_size;
        bias.data = bias_data + i * o_output->dim[1] / params->group;
        csi_conv2d_relu_i8(&input, &output, &kernel, &bias, &pparams);
    }
    return CSINN_TRUE;
}
                         ...
                         struct conv2d_params *params)
{
    if(params->wscales != NULL && params->wzps != NULL){
        if (params->layout == CSINN_NCHW) {
            if (params->group == 1) {
                params->bc = csi_bc_map(params->api, CSINN_OP_CONV2D_CHANNEL_RELU, input->dtype);
            } else if (params->group == output->dim[1]) {
                params->bc = csi_bc_map(params->api, CSINN_OP_DEPTHWISE_CONV2D_CHANNEL_RELU, input->dtype);
            } else {
                params->bc = csi_bc_map(params->api, CSINN_OP_GROUP_CONV2D_CHANNEL_RELU, input->dtype);
            }
            if (params->bc == NULL) {
    ...
    if (params->layout == CSINN_NCHW) {
        if (params->group == 1) {
            params->bc = csi_bc_map(params->api, CSINN_OP_CONV2D_RELU, input->dtype);
        } else if (params->group == output->dim[1]) {
            params->bc = csi_bc_map(params->api, CSINN_OP_DEPTHWISE_CONV2D_RELU, input->dtype);
        } else {
            params->bc = csi_bc_map(params->api, CSINN_OP_GROUP_CONV2D_RELU, input->dtype);
        }
        if (params->bc == NULL) {
            return CSINN_UNSUPPORT_DTYPE;
        }
