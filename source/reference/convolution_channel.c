...
static int csi_conv2d_channel_nchw_u8(struct csi_tensor *o_input,
                              ...
                              struct conv2d_params *params)
{
    ...
    for (int i = 0; i < o_kernel->dim[0]; i++) {
        ...
        for (int j = 0; j < per_cahnnel; j++) {
            ...
            float_kernel_data[index] = uint8_to_float_channel(kernel_data[index],
                                            params->wscales[i], params->wzps[i]);
        }
    }
    for (int i = 0; i < bias_size; i++) {
        float_bias_data[i] = bias_data[i] * params->wscales[i] * o_input->scale;
    }
    params->wscales = NULL;
    params->wzps = NULL;
    ...
    const int32_t *filter_offset = params->wzps;
    const float *filter_scales = params->wscales;
        ...
        for (int32_t out_y = 0; out_y < output_height; ++out_y) {
            for (int32_t out_x = 0; out_x < output_width; ++out_x) {
                for (int32_t out_channel = 0; out_channel < output_depth; ++out_channel) {
                        ...
                        for (int32_t filter_x = 0; filter_x < filter_width; ++filter_x) {
                            for (int32_t in_channel = 0; in_channel < input_depth; ++in_channel) {
                                ...
                                if ((in_x >= 0) && (in_x < input_width) && (in_y >= 0) &&
                                    ...
                                    acc +=
                                        (filter_val - filter_offset[out_channel]) * (input_val - input_offset);
                                }
                            }
                        }
                    }
                    ...
                    acc = csi_quantize_channel_u8(acc, input, output, filter_scales[out_channel]);
...
static int csi_conv2d_channel_relu_nchw_u8(struct csi_tensor *o_input,
                                   struct csi_tensor *o_output,
                                   struct csi_tensor *o_kernel,
                                   struct csi_tensor *o_bias,
                                   struct conv2d_params *params)
        ...
        for (int32_t out_y = 0; out_y < output_height; ++out_y) {
            for (int32_t out_x = 0; out_x < output_width; ++out_x) {
                for (int32_t out_channel = 0; out_channel < output_depth; ++out_channel) {
                    ...
                    acc = conv_channel_out_u8(acc, input, output, filter_scales[out_channel]);
                    output_data[csi_get_index(output->dim, batch, out_y, out_x, out_channel)] = acc;
                }
            }
        }
...
static int csi_conv2d_channel_relu6_nchw_u8(struct csi_tensor *o_input,
                                    struct csi_tensor *o_output,
                                    struct csi_tensor *o_kernel,
                                    struct csi_tensor *o_bias,
                                    struct conv2d_params *params)
        ...
        for (int32_t out_y = 0; out_y < output_height; ++out_y) {
            for (int32_t out_x = 0; out_x < output_width; ++out_x) {
                for (int32_t out_channel = 0; out_channel < output_depth; ++out_channel) {
                    ...
                    acc = conv_channel_relu6_u8(acc, input, output, filter_scales[out_channel]);
                    output_data[csi_get_index(output->dim, batch, out_y, out_x, out_channel)] = acc;
                }
            }
        }
...
static int csi_depthwise_conv2d_channel_nchw_u8(struct csi_tensor *o_input,
                                        struct csi_tensor *o_output,
                                        struct csi_tensor *o_kernel,
                                        struct csi_tensor *o_bias,
                                        struct conv2d_params *params)
            ...
            for (int32_t out_x = 0; out_x < output_width; ++out_x) {
                for (int32_t ic = 0; ic < input_depth; ++ic) {
                    for (int32_t m = 0; m < depth_multiplier; m++) {
                        ...
                        for (int32_t filter_y = 0; filter_y < filter_height; ++filter_y) {
                            for (int32_t filter_x = 0; filter_x < filter_width; ++filter_x) {
                                ...
                                if ((in_x >= 0) && (in_x < input_width) && (in_y >= 0) &&
                                    ...
                                    acc +=
                                        (filter_val - filter_offset[oc]) * (input_val - input_offset);
                                }
                            }
                        }
                        if (bias->dim_count != 0) {
                        ...
                        uint8_t out = csi_quantize_channel_u8(acc, input, output, filter_scales[oc]);
                        output_data[csi_get_index(output->dim, b, out_y, out_x, oc)] = out;
                    }
                }
            }
        }
...
static int csi_depthwise_conv2d_channel_relu_nchw_u8(struct csi_tensor *o_input,
                                                     struct csi_tensor *o_output,
                                                     struct csi_tensor *o_kernel,
                                                     struct csi_tensor *o_bias,
                                                     struct conv2d_params *params)
            ...
            for (int32_t out_x = 0; out_x < output_width; ++out_x) {
                for (int32_t ic = 0; ic < input_depth; ++ic) {
                    for (int32_t m = 0; m < depth_multiplier; m++) {
                            ...
                            for (int32_t filter_x = 0; filter_x < filter_width; ++filter_x) {
                                ...
                                if ((in_x >= 0) && (in_x < input_width) && (in_y >= 0) &&
                                    ...
                                    int32_t filter_val = kernel_data[csi_get_index(
                                        kernel->dim, ic, filter_y, filter_x, m)];
                                    acc +=
                                        (filter_val - filter_offset[ic]) * (input_val - input_offset);
                                }
                            }
                        }
                        if (bias->dim_count != 0) {
                        ...
                        acc = conv_channel_out_u8(acc, input, output, filter_scales[ic]);
                        output_data[csi_get_index(output->dim, b, out_y, out_x, oc)] = acc;
                    }
                }
            }
        }
...
static int csi_depthwise_conv2d_channel_relu6_nchw_u8(struct csi_tensor *o_input,
                                                     struct csi_tensor *o_output,
                                                     struct csi_tensor *o_kernel,
                                                     struct csi_tensor *o_bias,
                                                     struct conv2d_params *params)
            ...
            for (int32_t out_x = 0; out_x < output_width; ++out_x) {
                for (int32_t ic = 0; ic < input_depth; ++ic) {
                    for (int32_t m = 0; m < depth_multiplier; m++) {
                        ...
                        acc = conv_channel_relu6_u8(acc, input, output, filter_scales[oc]);
                        output_data[csi_get_index(output->dim, b, out_y, out_x, oc)] = acc;
                    }
                }
            }
...
static int csi_group_conv2d_channel_nchw_u8(struct csi_tensor *o_input,
                                    struct csi_tensor *o_output,
                                    struct csi_tensor *o_kernel,
                                    struct csi_tensor *o_bias,
                                    struct conv2d_params *params)
{
    ...
    for (int i = 0; i < params->group; i++) {
        ...
        pparams.wscales = params->wscales + i * o_output->dim[1] / params->group;
        pparams.wzps = params->wzps + i * o_output->dim[1] / params->group;

        csi_conv2d_channel_nchw_u8(&input, &output, &kernel, &bias, &pparams);
    }
    return CSINN_TRUE;
}


static int csi_group_conv2d_channel_relu_nchw_u8(struct csi_tensor *o_input,
                                         struct csi_tensor *o_output,
                                         struct csi_tensor *o_kernel,
                                         struct csi_tensor *o_bias,
                                         struct conv2d_params *params)
{
    ...
    for (int i = 0; i < params->group; i++) {
        ...
        pparams.wscales = params->wscales + i * o_output->dim[1] / params->group;
        pparams.wzps = params->wzps + i * o_output->dim[1] / params->group;
        csi_conv2d_channel_relu_nchw_u8(&input, &output, &kernel, &bias, &pparams);
    }
    return CSINN_TRUE;
}
                  ...
                  struct conv2d_params *params)
{
    if (params->layout == CSINN_NCHW) {
        csi_conv2d_channel_nchw_u8(input, output, kernel, bias, params);
    } else {
...
int csi_conv2d_channel_relu_u8(struct csi_tensor *input,
                  struct csi_tensor *output,
                  struct csi_tensor *kernel,
                  struct csi_tensor *bias,
                  struct conv2d_params *params)
{
    if (params->layout == CSINN_NCHW) {
        csi_conv2d_channel_relu_nchw_u8(input, output, kernel, bias, params);
    } else {
        return CSINN_UNSUPPORT_LAYOUT;
    }
}
...
{
    if (params->layout == CSINN_NCHW) {
        csi_conv2d_channel_relu6_nchw_u8(input, output, kernel, bias, params);
    } else {
        return CSINN_UNSUPPORT_LAYOUT;
    }
}
...
{
    if (params->layout == CSINN_NCHW) {
        csi_depthwise_conv2d_channel_nchw_u8(input, output, kernel, bias, params);
    } else {
        return CSINN_UNSUPPORT_LAYOUT;
    }
}

int csi_depthwise_conv2d_channel_relu_u8(struct csi_tensor *input,
                            struct csi_tensor *output,
                            struct csi_tensor *kernel,
                            struct csi_tensor *bias,
                            struct conv2d_params *params)
{
    if (params->layout == CSINN_NCHW) {
        csi_depthwise_conv2d_channel_relu_nchw_u8(input, output, kernel, bias, params);
    } else {
        return CSINN_UNSUPPORT_LAYOUT;
    }
}
...
{
    if (params->layout == CSINN_NCHW) {
        csi_depthwise_conv2d_channel_relu6_nchw_u8(input, output, kernel, bias, params);
    } else {
        return CSINN_UNSUPPORT_LAYOUT;
    }
}
...
{
    if (params->layout == CSINN_NCHW) {
        csi_group_conv2d_channel_nchw_u8(input, output, kernel, bias, params);
    } else {
        return CSINN_UNSUPPORT_LAYOUT;
    }
}
...
{
    if (params->layout == CSINN_NCHW) {
        csi_group_conv2d_channel_relu_nchw_u8(input, output, kernel, bias, params);
    } else {
        return CSINN_UNSUPPORT_LAYOUT;
    }
}