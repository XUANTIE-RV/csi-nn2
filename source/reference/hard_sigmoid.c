...
static int csi_hard_sigmoid_f32(struct csi_tensor *input,
...
static int csi_hard_sigmoid_u8(struct csi_tensor *input,
                                struct csi_tensor *output,
                                struct sigmoid_params *params)
{
    ...
    for(int i = 0; i < size; i++) {
        ...
        output_data[i] = csi_quantize_f32(output_val, output->offset, output->multiplier, output->shift);
    }
    return CSINN_TRUE;
}
                        ...
                        struct sigmoid_params *params)
{
    if(input->dtype == CSINN_DTYPE_UINT8) {
        params->bc = csi_hard_sigmoid_u8;
    } else if(input->dtype == CSINN_DTYPE_FLOAT32) {
        params->bc = csi_hard_sigmoid_f32;
    } else {
