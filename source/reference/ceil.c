...
static int csi_ceil_f32(struct csi_tensor *input,
                         ...
                         struct siso_params *params)
{
    ...
    for (int i = 0; i < size; i++) {
        output_data[i] = ceil(input_data[i]);
    }

    return CSINN_TRUE;
}

static int csi_ceil_u8(struct csi_tensor *input,
                        struct csi_tensor *output,
                        struct siso_params *params)
{
                  ...
                  struct siso_params *params)
{
    if (input->dtype == CSINN_DTYPE_UINT8) {
        params->bc = csi_ceil_u8;
    } else if (input->dtype == CSINN_DTYPE_FLOAT32) {
        params->bc = csi_ceil_f32;
    } else {
