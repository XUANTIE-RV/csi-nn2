...
static int csi_l2_normalization_f32(struct csi_tensor *input,
...
static int csi_l2_normalization_u8(struct csi_tensor *input,
                              struct csi_tensor *output,
                              struct l2n_params *params)
{
    ...
    for (int i = 0; i < outer_size; ++i) {
        float squared_l2_norm = 0;
        for (int c = 0; c < depth; ++c) {
            const float val = csi_dequantize_f32(input_data[depth * i + c], input->offset,
                                            input->multiplier, input->shift);
            squared_l2_norm += val * val;
        }
        const float l2_norm = sqrt(squared_l2_norm + params->epsilon);
        for (int c = 0; c < depth; ++c) {
            output_data[depth * i + c] = csi_quantize_f32(input_data[depth * i + c] / l2_norm, output->offset,
                 ...
                 struct l2n_params *params)
{
    if (input->dtype == CSINN_DTYPE_FLOAT32) {
        params->bc = csi_l2_normalization_f32;
    } else if (input->dtype == CSINN_DTYPE_UINT8) {
        params->bc = csi_l2_normalization_u8;
    } else {
