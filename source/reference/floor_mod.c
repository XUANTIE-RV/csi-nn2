...
static int csi_floor_mod_f32(struct csi_tensor *input0,
                             ...
                             struct diso_params *params)
{
    ...
    for (int i = 0; i < size; i++) {
        float res = input0_data[i] - floor(input0_data[i] / input1_data[i]) * input1_data[i];
        // res = floor(res);
        output_data[i] = res;
    }
    return CSINN_TRUE;
...
static int csi_floor_mod_u8(struct csi_tensor *input0,
                            struct csi_tensor *input1,
                            struct csi_tensor *output,
                            struct diso_params *params)
{
    ...
    for (int i = 0; i < size; i++) {
        ...
        float input1_val = csi_dequantize_f32(input1_data[i], input1->offset, input1->multiplier,
                                               input1->shift);
        float res = input0_val - floor(input0_val / input1_val) * input1_val;
        // res = floor(res);
                       ...
                       struct diso_params *params)
{
    if (input0->dtype == CSINN_DTYPE_UINT8) {
        params->bc = csi_floor_mod_u8;
    } else if (input0->dtype == CSINN_DTYPE_FLOAT32) {
        params->bc = csi_floor_mod_f32;
    } else {
