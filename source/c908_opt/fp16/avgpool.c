                                 ...
                                 struct csinn_pool_params *params)
{
    ...
    if (stride_h == 2 && stride_w == 2) {
        if (kernel_h == 2 && kernel_w == 2) {
            if (pad_left == 0 && pad_top == 0) {
                ...
                cb->exec = (elempack % packn == 0) ? shl_rvv_avgpool2x2s2_packn_fp16
            ...
            if (pad_left == 0 && pad_top == 0) {
                ...
                cb->exec = (elempack % packn == 0) ? shl_rvv_avgpool3x3s2_packn_fp16
        ...
        if (kernel_h == 3 && kernel_w == 3) {
            if (pad_left == 1 && pad_top == 1 && pad_right == 1 && pad_down == 1) {
                cb->exec = (elempack % packn == 0) ? shl_rvv_avgpool3x3s1_packn_fp16
    ...
    if (cb->exec == NULL) {
        if (elempack % packn == 0) {
            cb->exec = shl_rvv_avgpool_packn_fp16;
        } else {
            shl_debug_warning(
                "avgpool is not optimized to achieve under this condition on C908, call reference "
                "func replaced.\n");
