...
{
    static int i = 0;
    if (i >= C908_OP_PATTERN_MAX) {
        shl_debug_error("C908 callback length is greater than C908_OP_PATTERN_MAX!\n");
    }
...
void shl_c908_session_init(struct csinn_session *sess)
{
    ...
    sess->td = target_data;
    shl_c908_set_binary_model_op_init(sess, false);
...
static void sess_op_init(struct csinn_session *sess)
{
    ...
    shl_c908_set_packn_layout(sess, use_packn);
...
void shl_c908_session_setup(struct csinn_session *sess)
{
    struct shl_ref_graph *graph = shl_gref_get_graph(sess);
    struct shl_node *n;
    FILE *b;
...
int shl_c908_load_binary_model(struct csinn_session *sess)
{
    ...
    shl_c908_set_binary_model_op_init(sess, true);
    sess_op_init(sess);
...
void *shl_c908_runtime_callback(int api)
{
    switch (api) {
        ...
        case CSINN_SESSION_SETUP:
            return shl_c908_session_setup;
            break;
        case CSINN_LOAD_BG:
            return shl_c908_load_binary_model;
            break;
