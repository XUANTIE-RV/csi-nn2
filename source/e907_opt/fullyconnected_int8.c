                                                 ...
                                                 int out_nodes)
{
    ...
    for (int i = 0; i < out_nodes; i++) {
        ...
        int32_t acc = bias[i];
        int j = 0;
        for (; j + 3 < in_nodes; j += 4) {
            int32_t *input_8x4 = (int32_t *)(input + j);
            int32_t *weight_8x4 = (int32_t *)(weight_ptr + j);
            acc = __rv__smaqa(acc, input_8x4[0], weight_8x4[0]);
        }
        for (; j < in_nodes; j++) {
            acc += input[j] * weight_ptr[j];
        }
        output[i] = acc;
    }
#endif
