                   ...
                   struct diso_params *params)
{
    ...
    node = vsi_nn_AddNode(graph, VSI_NN_OP_SELECT, input_num, output_num, &node_id);
    attr.dtype.fmt = VSI_NN_DIM_FMT_NCHW;

    /* input */
    node->input.tensors[0] = (vsi_nn_tensor_id_t)condition->data;
    node->input.tensors[1] = (vsi_nn_tensor_id_t)input0->data;
    node->input.tensors[2] = (vsi_nn_tensor_id_t)input1->data;
