                      ...
                      struct unpooling_params *params)
{
    ...
    node = vsi_nn_AddNode(graph, VSI_NN_OP_UPSAMPLE, input_num, output_num, &node_id);
    node->nn_param.upsample.size[0] = 0;
    node->nn_param.upsample.size[1] = 0;
    node->nn_param.upsample.scale[0] = params->scale_width;
    node->nn_param.upsample.scale[1] = params->scale_height;

    attr.dtype.fmt = VSI_NN_DIM_FMT_NCHW;

    /* input */
    node->input.tensors[0] = (vsi_nn_tensor_id_t)input->data;
    node->input.tensors[1] = (vsi_nn_tensor_id_t)mask->data;
