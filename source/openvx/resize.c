                   ...
                   struct resize_params *params)
{
    ...
    node = vsi_nn_AddNode(graph, VSI_NN_OP_RESIZE, input_num, output_num, &node_id);
    node->nn_param.resize.factor = 0;
    node->nn_param.resize.align_corners = params->align_corners;
    node->nn_param.resize.size[0] = output->dim[3];
    node->nn_param.resize.size[1] = output->dim[2];
    node->nn_param.resize.size[2] = output->dim[1];
    node->nn_param.resize.size[3] = output->dim[0];
