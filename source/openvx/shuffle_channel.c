                            ...
                            struct shuffle_channel_params *params)
{
    ...
    node = vsi_nn_AddNode(graph, VSI_NN_OP_SHUFFLECHANNEL, input_num, output_num, &node_id);
    node->nn_param.shufflechannel.axis = input->dim_count - 1 - 1; // FIXME: input tensor layout (NCHW) channel axis
    node->nn_param.shufflechannel.group_number = params->group;
