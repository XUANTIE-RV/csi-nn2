                      ...
                      struct transpose_params *params)
{
    ...
    node = vsi_nn_AddNode(graph, VSI_NN_OP_PERMUTE, input_num, output_num, &node_id);
    uint32_t *perm = (uint32_t *)malloc(sizeof(uint32_t)*output->dim_count);
    int i = 0;
    for (i = 0; i < output->dim_count; i++) {
        perm[output->dim_count-1-i] = output->dim_count - 1 - params->permute[i];
    }
    node->nn_param.permute.perm = perm;
    node->nn_param.permute.dim_num = output->dim_count;
    node->nn_param.permute.local.initialized = FALSE;
