                   ...
                   struct gather_params *params)
{
    ...
    node = vsi_nn_AddNode(graph, VSI_NN_OP_GATHER, input_num, output_num, &node_id);
    node->nn_param.gather.axis = input->dim_count - 1 - params->axis;

    attr.dtype.fmt = VSI_NN_DIM_FMT_NCHW;

    /* input */
    node->input.tensors[0] = (vsi_nn_tensor_id_t)input->data;
    node->input.tensors[1] = (vsi_nn_tensor_id_t)indices->data;
