...
static inline vfloat32m8_t vdeq_vf_f32m8(vint8m2_t _src, __fp16 scale, int vl)
{
    vint16m4_t _i16 = vwadd_vx_i16m4(_src, 0, vl);
    vfloat16m4_t _f16 = vfcvt_f_x_v_f16m4(_i16, vl);
    _f16 = vfmul_vf_f16m4(_f16, scale, vl);
    vfloat32m8_t _f32 = vfwcvt_f_f_v_f32m8(_f16, vl);
    return _f32;
}
 ...
 ************************************************************/
static inline void gemm_dot_1x1_fp32_q8(float *dst, const float *sa, const int8_t *sb,
                                        const __fp16 *scale, float *bias, int M, int K, int N,
                                        int lda, int ldb, int ldc, int k_idx)
{
    ...
    for (; i < M; i++) {
        ...
        for (; j < N; j++) {
            ...
            // vlen128 e32m8=32
            int vl = vsetvl_e32m8(block_size);
            // dst[0, 0]
            vfloat32m8_t _acc00 = vfmv_v_f_f32m8(0.0f, vl);

            int c = 0;
            for (; c + block_size - 1 < K; c += block_size) {
                vfloat32m8_t _a0 = vle32_v_f32m8(a0_ptr + c, vl);
                vint8m2_t _b0_i8 = vle8_v_i8m2(b0_ptr + c, vl);
                vfloat32m8_t _b0_f32 = vdeq_vf_f32m8(_b0_i8, s0_ptr[0], vl);
                s0_ptr += 1;
                _acc00 = vfmacc_vv_f32m8(_acc00, _a0, _b0_f32, vl);
            }

            int idx00 = (i + 0) * ldc + (j + 0);
            vfloat32m1_t _sum00;
            if (k_idx == 0) {
                _sum00 = vfmv_v_f_f32m1(bias[j + 0], 1);
            } else {
                _sum00 = vfmv_v_f_f32m1(dst[idx00], 1);
            }

            _sum00 = vfredosum_vs_f32m8_f32m1(vundefined_f32m1(), _acc00, _sum00, vl);
            dst[idx00] = vfmv_f_s_f32m1_f32(_sum00);
        }
    }
}

static void gemm_dot_1x1_fp32_q8_omp(float *dst, const float *sa, const int8_t *sb,
...
#pragma omp parallel
        {
            ...
            float *thread_dst = dst + M_start * ldc + N_start;
            const float *thread_sa = sa + M_start * lda;
            const int8_t *thread_sb = sb + N_start * ldb;
            const __fp16 *thread_scale = scale + N_start * ldb / 32;
            float *thread_bias = bias + N_start;
            int thread_M = M_end - M_start;
            int thread_N = N_end - N_start;
            gemm_dot_1x1_fp32_q8(thread_dst, thread_sa, thread_sb, thread_scale, thread_bias,
                                 thread_M, K, thread_N, lda, ldb, ldc, k_idx);
        }
    } else {
        gemm_dot_1x1_fp32_q8(dst, sa, sb, scale, bias, M, K, N, lda, ldb, ldc, k_idx);
    }
}
 ...
 * constrain: vlen = 128, and K % 32 == 0
 ************************************************************/
static inline void gemm_dot_1x1_fp32_q4(float *dst, const float *sa, const int8_t *sb,
                                        const __fp16 *scale, float *bias, int M, int K, int N,
                                        int lda, int ldb, int ldc, int k_idx)
{
    int block_size = 32;
    ...
    for (; i < M; i++) {
        ...
        for (; j < N; j++) {
            ...
            // vlen128 e32m4=16
            int vl = vsetvl_e32m4(half_block);
            // dst[0, 0]
            vfloat32m4_t _acc00 = vfmv_v_f_f32m4(0.0f, vl);

            int c = 0;
            for (; c + block_size - 1 < K; c += block_size) {
                vfloat32m4_t _a00 = vle32_v_f32m4(a0_ptr + c, vl);
                vfloat32m4_t _a01 = vle32_v_f32m4(a0_ptr + c + half_block, vl);
                ...
                s0_ptr += 1;
                vfloat32m4_t _low_f32 = vfwcvt_f_f_v_f32m4(_low_f16, vl);
                vfloat32m4_t _high_f32 = vfwcvt_f_f_v_f32m4(_high_f16, vl);

                _acc00 = vfmacc_vv_f32m4(_acc00, _a00, _low_f32, vl);
                _acc00 = vfmacc_vv_f32m4(_acc00, _a01, _high_f32, vl);
            }
...
static void gemm_dot_1x1_fp32_q4_omp(float *dst, const float *sa, const int8_t *sb,
                                     const __fp16 *scale, float *bias, int M, int K, int N, int lda,
                                     int ldb, int ldc, int k_idx)
{
    if (shl_multithread_is_enable()) {
#pragma omp parallel
        {
            ...
            int thread_M = M_end - M_start;
            int thread_N = N_end - N_start;
            gemm_dot_1x1_fp32_q4(thread_dst, thread_sa, thread_sb, thread_scale, thread_bias,
                                 thread_M, K, thread_N, lda, ldb, ldc, k_idx);
        }
    } else {
        gemm_dot_1x1_fp32_q4(dst, sa, sb, scale, bias, M, K, N, lda, ldb, ldc, k_idx);
    }
}

static inline void gemm_dot_4x4_fp32(float *dst, const float *sa, const float *sb, float *bias,
                                     int M, int K, int N, int lda, int ldb, int ldc, int k_idx)
{
    ...
    for (; i + 3 < M; i += 4) {
        ...
        for (; j + 3 < N; j += 4) {
            ...
            if (k_idx == 0) {
                _sum00 = vfmv_v_f_f32m1(bias[j + 0], 1);
                _sum10 = vmv_v_v_f32m1(_sum00, 1);
                _sum20 = vmv_v_v_f32m1(_sum00, 1);
                _sum30 = vmv_v_v_f32m1(_sum00, 1);

                _sum01 = vfmv_v_f_f32m1(bias[j + 1], 1);
                _sum11 = vmv_v_v_f32m1(_sum01, 1);
                _sum21 = vmv_v_v_f32m1(_sum01, 1);
                _sum31 = vmv_v_v_f32m1(_sum01, 1);

                _sum02 = vfmv_v_f_f32m1(bias[j + 2], 1);
                _sum12 = vmv_v_v_f32m1(_sum02, 1);
                _sum22 = vmv_v_v_f32m1(_sum02, 1);
                _sum32 = vmv_v_v_f32m1(_sum02, 1);

                _sum03 = vfmv_v_f_f32m1(bias[j + 3], 1);
                _sum13 = vmv_v_v_f32m1(_sum03, 1);
                _sum23 = vmv_v_v_f32m1(_sum03, 1);
                _sum33 = vmv_v_v_f32m1(_sum03, 1);
            } else {
                _sum00 = vfmv_v_f_f32m1(dst[idx00], 1);
...
static void gemm_dot_4x4_fp32_omp(float *dst, const float *sa, const float *sb, float *bias, int M,
...
#pragma omp parallel
        {
            ...
            float *thread_dst = dst + M_start * ldc + N_start;
            const float *thread_sa = sa + M_start * lda;
            const float *thread_sb = sb + N_start * ldb;
            float *thread_bias = bias + N_start;
            int thread_M = M_end - M_start;
            int thread_N = N_end - N_start;
            gemm_dot_4x4_fp32(thread_dst, thread_sa, thread_sb, thread_bias, thread_M, K, thread_N,
                              lda, ldb, ldc, k_idx);
        }
    } else {
        gemm_dot_4x4_fp32(dst, sa, sb, bias, M, K, N, lda, ldb, ldc, k_idx);
    }
}
...
static inline void c920_get_blk_size(int M, int N, int K, int *m_blk, int *n_blk, int *k_blk)
{
    ...
    const int N_BLK = 256;
    const int K_BLK = 370;
    const int CACHE_SIZE = 1024 * 1024 / sizeof(float) * 0.75;
 ...
 * constrain: vlen >= 128, and K % 32 == 0
 ************************************************************/
static void dequantize_block_q8_to_f32(const int8_t *src, const __fp16 *scale, float *dst,
                                      ...
                                      int M, int K, int N, const __fp16 *scale)
{
    ...
    if (M > 1) {
        ...
        float *b_fp32 = (float *)shl_mem_alloc(N_BLK * K_BLK * sizeof(float));
        int lda = K;
        ...
        while (m_idx < M) {
            ...
            while (n_idx < N) {
                ...
                while (k_idx < K) {
                    ...
                    float *c_ptr = dst + m_idx * N + n_idx;
                    const float *a_ptr = sa + m_idx * K + k_idx;
                    const int8_t *b_ptr = sb + n_idx * K + k_idx;
                    const __fp16 *scale_ptr = scale + n_idx * (K / 32) + k_idx / 32;

                    // dequantize before gemm
                    dequantize_block_q8_to_f32(b_ptr, scale_ptr, b_fp32, n_block, k_block, K,
                                               K_BLK);
                    gemm_dot_4x4_fp32_omp(c_ptr, a_ptr, b_fp32, bias + n_idx, m_block, k_block,
        ...
        int ldc = N;
        // dequantize in gemm
        gemm_dot_1x1_fp32_q8_omp(dst, sa, sb, scale, bias, M, K, N, lda, ldb, ldc, 0);
    }
 ...
 * constrain: vlen >= 128, and K % 32 == 0
 ************************************************************/
static void dequantize_block_q4_to_f32(const int8_t *src, const __fp16 *scale, float *dst,
                                       int n_blk, int k_blk, int ld_src, int ld_dst)
{
    int block_size = 32;
    int half_block = block_size / 2;
    ...
    for (int i = 0; i < n_blk; i++) {
        ...
        for (int j = 0; j + block_size - 1 < k_blk; j += block_size) {
            ...
            vfloat32m4_t _low_f32 = vfwcvt_f_f_v_f32m4(_low_f16, vl);
            vfloat32m4_t _high_f32 = vfwcvt_f_f_v_f32m4(_high_f16, vl);
            vse32_v_f32m4(d_ptr, _low_f32, vl);
            vse32_v_f32m4(d_ptr + half_block, _high_f32, vl);
            d_ptr += block_size;
        }
    }
}
                                      ...
                                      int M, int K, int N, const __fp16 *scale)
{
    ...
    if (M > 1) {
        ...
        while (m_idx < M) {
            ...
            while (n_idx < N) {
                ...
                while (k_idx < K) {
                    ...
                    // dequantize before gemm
                    dequantize_block_q4_to_f32(b_ptr, scale_ptr, b_fp32, n_block, k_block, K,
                                               K_BLK);
                    gemm_dot_4x4_fp32_omp(c_ptr, a_ptr, b_fp32, bias + n_idx, m_block, k_block,
                                          n_block, lda, ldb, ldc, k_idx);
        ...
        int ldc = N;
        // dequantize in gemm
        gemm_dot_1x1_fp32_q4_omp(dst, sa, sb, scale, bias, M, K, N, lda, ldb, ldc, 0);
    }
