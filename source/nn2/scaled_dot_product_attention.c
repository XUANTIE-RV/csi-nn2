...
{
    shl_op_callback_map(&params->base, CSINN_OP_SCALED_DOT_PRODUCT_ATTENTION, query->dtype);
    int (*func)() = shl_get_init_cb(&params->base);
    if (func != NULL) {
        func(query, key, value, output, params);
    }
    return CSINN_TRUE;
}
/**
...
{
    SHL_DEBUG_CALL(
        shl_scaled_dot_product_attention_debug_info(query, key, value, output, params, __func__));
