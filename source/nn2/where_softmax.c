...
{
    shl_op_callback_map(&params->base, CSINN_OP_WHERE_SOFTMAX, output->dtype);
    int (*func)() = shl_get_init_cb(&params->base);
    if (func != NULL) {
        func(condition, y, output, params);
    }
    return CSINN_TRUE;
}
/**
...
int csinn_where_softmax(struct csinn_tensor *condition, struct csinn_tensor *y,
                        struct csinn_tensor *output, struct csinn_where_softmax_params *params)
{
    SHL_DEBUG_CALL(shl_where_softmax_debug_info(condition, y, output, params, __func__));
