...
static void avgpool_w8_int8_nhwc(const int8_t *src, int8_t *dst, struct csinn_pool_params *params,
                                 int oh, int ow, int idx_h_start, int idx_h_end, int in_w,
                                 int out_w, int in_c, enum avgpool_loc_enum loc, __fp16 real_scale,
                                 int z1, int z2)
{
    ...
    while (c < in_c) {
        ...
        vse8_v_i8m1(out_ptr + 0 * in_c, vnclip_wx_i8m1(_res0, 0, vl), vl);
        vse8_v_i8m1(out_ptr + 1 * in_c, vnclip_wx_i8m1(_res1, 0, vl), vl);
        vse8_v_i8m1(out_ptr + 2 * in_c, vnclip_wx_i8m1(_res2, 0, vl), vl);
        vse8_v_i8m1(out_ptr + 3 * in_c, vnclip_wx_i8m1(_res3, 0, vl), vl);
        vse8_v_i8m1(out_ptr + 4 * in_c, vnclip_wx_i8m1(_res4, 0, vl), vl);
        vse8_v_i8m1(out_ptr + 5 * in_c, vnclip_wx_i8m1(_res5, 0, vl), vl);
        vse8_v_i8m1(out_ptr + 6 * in_c, vnclip_wx_i8m1(_res6, 0, vl), vl);
        vse8_v_i8m1(out_ptr + 7 * in_c, vnclip_wx_i8m1(_res7, 0, vl), vl);
        c += vl;
    }
}

static void avgpool_border_int8_nhwc(const int8_t *src, int8_t *dst,
                              ...
                              struct csinn_pool_params *params)
{
    ...
    for (int b = 0; b < batch; b++) {
        const int8_t *in_ptr = input_data + b * in_h * in_w * in_c;
        int8_t *out_ptr = output_data + b * out_h * out_w * out_c;
        ...
        for (; oh < dst_start_h; oh++) {
            ...
            for (; ow < dst_1x8_start_w; ow++) {
                avgpool_border_int8_nhwc(in_ptr, out_ptr, params, oh, ow, idx_h_start, idx_h_end,
                                         in_w, out_w, in_c, AVGPOOL_LEFT_TOP, real_scale, z1, z2);
            }
            for (; ow + 7 < dst_1x8_end_w; ow += 8) {
                avgpool_w8_int8_nhwc(in_ptr, out_ptr, params, oh, ow, idx_h_start, idx_h_end, in_w,
                                     out_w, in_c, AVGPOOL_TOP, real_scale, z1, z2);
            }
            for (; ow < out_w; ow++) {
                avgpool_border_int8_nhwc(in_ptr, out_ptr, params, oh, ow, idx_h_start, idx_h_end,
                                         in_w, out_w, in_c, AVGPOOL_RIGHT_TOP, real_scale, z1, z2);
            }
        }
        for (; oh < dst_end_h; oh++) {
            int in_h_start = -pad_top + oh * stride_h;
            ...
            for (; ow < dst_1x8_start_w; ow++) {
                avgpool_border_int8_nhwc(in_ptr, out_ptr, params, oh, ow, idx_h_start, idx_h_end,
                                         in_w, out_w, in_c, AVGPOOL_LEFT, real_scale, z1, z2);
            }
            for (; ow + 7 < dst_1x8_end_w; ow += 8) {
                avgpool_w8_int8_nhwc(in_ptr, out_ptr, params, oh, ow, idx_h_start, idx_h_end, in_w,
                                     out_w, in_c, AVGPOOL_CENTER, real_scale, z1, z2);
            }
            for (; ow < out_w; ow++) {
                avgpool_border_int8_nhwc(in_ptr, out_ptr, params, oh, ow, idx_h_start, idx_h_end,
                                         in_w, out_w, in_c, AVGPOOL_RIGHT, real_scale, z1, z2);
            }
        }
        for (; oh < out_h; oh++) {
            int in_h_start = -pad_top + oh * stride_h;
            ...
            for (; ow < dst_1x8_start_w; ow++) {
                avgpool_border_int8_nhwc(in_ptr, out_ptr, params, oh, ow, idx_h_start, idx_h_end,
                                         in_w, out_w, in_c, AVGPOOL_LEFT_BOTTOM, real_scale, z1,
                                         z2);
            }
            for (; ow + 7 < dst_1x8_end_w; ow += 8) {
                avgpool_w8_int8_nhwc(in_ptr, out_ptr, params, oh, ow, idx_h_start, idx_h_end, in_w,
                                     out_w, in_c, AVGPOOL_BOTTOM, real_scale, z1, z2);
            }
            for (; ow < out_w; ow++) {
                avgpool_border_int8_nhwc(in_ptr, out_ptr, params, oh, ow, idx_h_start, idx_h_end,
                                         in_w, out_w, in_c, AVGPOOL_RIGHT_BOTTOM, real_scale, z1,
                                         z2);
            }
        }
    }
