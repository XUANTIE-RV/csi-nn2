...
static void avgpool_w8_fp16_nhwc(const __fp16 *src, __fp16 *dst, struct csinn_pool_params *params,
    ...
    while (c < in_c) {
        ...
        for (int h = idx_h_start; h < idx_h_end; h++) {
            for (int w = idx_w_start; w < idx_w_end; w++) {
                ...
                _acc0 = vfadd_vv_f16m1(_acc0, vle16_v_f16m1(in_ptr + 0 * stride_w * in_c, vl), vl);
                _acc1 = vfadd_vv_f16m1(_acc1, vle16_v_f16m1(in_ptr + 1 * stride_w * in_c, vl), vl);
                _acc2 = vfadd_vv_f16m1(_acc2, vle16_v_f16m1(in_ptr + 2 * stride_w * in_c, vl), vl);
                _acc3 = vfadd_vv_f16m1(_acc3, vle16_v_f16m1(in_ptr + 3 * stride_w * in_c, vl), vl);
                _acc4 = vfadd_vv_f16m1(_acc4, vle16_v_f16m1(in_ptr + 4 * stride_w * in_c, vl), vl);
                _acc5 = vfadd_vv_f16m1(_acc5, vle16_v_f16m1(in_ptr + 5 * stride_w * in_c, vl), vl);
                _acc6 = vfadd_vv_f16m1(_acc6, vle16_v_f16m1(in_ptr + 6 * stride_w * in_c, vl), vl);
                _acc7 = vfadd_vv_f16m1(_acc7, vle16_v_f16m1(in_ptr + 7 * stride_w * in_c, vl), vl);
            }
        }

        __fp16 *out_ptr = dst + (oh * out_w + ow) * in_c + c;
        vse16_v_f16m1(out_ptr + 0 * in_c, vfmul_vf_f16m1(_acc0, ratio, vl), vl);
        vse16_v_f16m1(out_ptr + 1 * in_c, vfmul_vf_f16m1(_acc1, ratio, vl), vl);
        vse16_v_f16m1(out_ptr + 2 * in_c, vfmul_vf_f16m1(_acc2, ratio, vl), vl);
        vse16_v_f16m1(out_ptr + 3 * in_c, vfmul_vf_f16m1(_acc3, ratio, vl), vl);
        vse16_v_f16m1(out_ptr + 4 * in_c, vfmul_vf_f16m1(_acc4, ratio, vl), vl);
        vse16_v_f16m1(out_ptr + 5 * in_c, vfmul_vf_f16m1(_acc5, ratio, vl), vl);
        vse16_v_f16m1(out_ptr + 6 * in_c, vfmul_vf_f16m1(_acc6, ratio, vl), vl);
        vse16_v_f16m1(out_ptr + 7 * in_c, vfmul_vf_f16m1(_acc7, ratio, vl), vl);

        c += vl;
    }
}

static void avgpool_border_fp16_nhwc(const __fp16 *src, __fp16 *dst,
                              ...
                              struct csinn_pool_params *params)
{
    ...
    for (int b = 0; b < batch; b++) {
        const __fp16 *in_ptr = input_data + b * in_h * in_w * in_c;
        ...
        for (; oh < dst_start_h; oh++) {
            ...
            for (; ow < dst_1x8_start_w; ow++) {
                avgpool_border_fp16_nhwc(in_ptr, out_ptr, params, oh, ow, idx_h_start, idx_h_end,
                                         in_w, out_w, in_c, AVGPOOL_LEFT_TOP);
            }
            for (; ow + 7 < dst_1x8_end_w; ow += 8) {
                avgpool_w8_fp16_nhwc(in_ptr, out_ptr, params, oh, ow, idx_h_start, idx_h_end, in_w,
