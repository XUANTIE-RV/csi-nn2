                          ...
                          struct csinn_tensor *weight, struct csinn_rms_norm_params *params)
{
    ...
    for (int b = 0; b < batches; b++) {
        ...
        vfloat16m1_t _sum = vfmv_v_f_f16m1(0.0f, 1);
        int i = 0;
        while (i < norm_size) {
            int vl = vsetvl_e16m2(norm_size - i);
            vfloat16m2_t _x = vle16_v_f16m2(input_ptr + i, vl);
            vfloat16m2_t _x2 = vfmul_vv_f16m2(_x, _x, vl);
            _sum = vfredosum_vs_f16m2_f16m1(vundefined_f16m1(), _x2, _sum, vl);
            i += vl;
        }

        float sum = vfmv_f_s_f16m1_f16(_sum);
        ...
        while (i < norm_size) {
            ...
            vfloat16m2_t _res = vfmul_vf_f16m2(_x, scale, vl);
            _res = vfmul_vv_f16m2(_res, _w, vl);
