                                ...
                                struct csinn_pool_params *params)
{
    ...
    if (input->layout == CSINN_LAYOUT_NCHW) {
        ...
        if (in_h == kernel_h && in_w == kernel_w) {
            cb->exec = (elempack % packn == 0) ? shl_rvv_global_maxpool2d_packn_fp32
            ...
            if (stride_h == 2 && stride_w == 2) {
                if (kernel_h == 2 && kernel_w == 2) {  // 2x2s2
                    if (pad_left == 0 && pad_top == 0) {
                        ...
                        cb->exec = shl_rvv_maxpool2x2s2_fp32;
                    } else if (pad_left == 1 && pad_top == 1) {
                        cb->exec = shl_rvv_maxpool2x2s2_p1_fp32;
                    }
                    ...
                    if (pad_left == 0 && pad_top == 0) {
                        ...
                        cb->exec = shl_rvv_maxpool3x3s2_fp32;
                    } else if (pad_left == 1 && pad_top == 1) {
                        if (params->ceil_mode == 0) {
                            cb->exec = shl_rvv_maxpool3x3s2_p1_fp32;
                        } else {
                ...
                if (kernel_h == 3 && kernel_w == 3) {
                    if (pad_left == 1 && pad_top == 1 && pad_right == 1 && pad_down == 1) {
                        cb->exec = shl_rvv_maxpool3x3s1_p1_fp32;
                    }
                }
            }
        }
        ...
        // global maxpool2d
        if (in_h == kernel_h && in_w == kernel_w) {
            cb->exec = shl_rvv_global_maxpool2d_nhwc_fp32;
            return CSINN_TRUE;
        }
        cb->exec = shl_rvv_maxpool_nhwc_fp32;
    }
