...
static void maxpool_w8_fp32_nhwc(const float *src, float *dst, struct csinn_pool_params *params,
    ...
    while (c < in_c) {
        ...
        for (int h = idx_h_start; h < idx_h_end; h++) {
            for (int w = idx_w_start; w < idx_w_end; w++) {
                ...
                _max0 = vfmax_vv_f32m1(_max0, vle32_v_f32m1(in_ptr + 0 * stride_w * in_c, vl), vl);
                _max1 = vfmax_vv_f32m1(_max1, vle32_v_f32m1(in_ptr + 1 * stride_w * in_c, vl), vl);
                _max2 = vfmax_vv_f32m1(_max2, vle32_v_f32m1(in_ptr + 2 * stride_w * in_c, vl), vl);
                _max3 = vfmax_vv_f32m1(_max3, vle32_v_f32m1(in_ptr + 3 * stride_w * in_c, vl), vl);
                _max4 = vfmax_vv_f32m1(_max4, vle32_v_f32m1(in_ptr + 4 * stride_w * in_c, vl), vl);
                _max5 = vfmax_vv_f32m1(_max5, vle32_v_f32m1(in_ptr + 5 * stride_w * in_c, vl), vl);
                _max6 = vfmax_vv_f32m1(_max6, vle32_v_f32m1(in_ptr + 6 * stride_w * in_c, vl), vl);
                _max7 = vfmax_vv_f32m1(_max7, vle32_v_f32m1(in_ptr + 7 * stride_w * in_c, vl), vl);
            }
        }
        float *out_ptr = dst + (oh * out_w + ow) * in_c + c;
        vse32_v_f32m1(out_ptr + 0 * in_c, _max0, vl);
        vse32_v_f32m1(out_ptr + 1 * in_c, _max1, vl);
        vse32_v_f32m1(out_ptr + 2 * in_c, _max2, vl);
        vse32_v_f32m1(out_ptr + 3 * in_c, _max3, vl);
        vse32_v_f32m1(out_ptr + 4 * in_c, _max4, vl);
        vse32_v_f32m1(out_ptr + 5 * in_c, _max5, vl);
        vse32_v_f32m1(out_ptr + 6 * in_c, _max6, vl);
        vse32_v_f32m1(out_ptr + 7 * in_c, _max7, vl);
        c += vl;
    }
}

static void maxpool_border_fp32_nhwc(const float *src, float *dst, struct csinn_pool_params *params,
                                     int oh, int ow, int idx_h_start, int idx_h_end, int in_w,
                                     int out_w, int in_c)
{
    int kernel_w = params->filter_width;
                              ...
                              struct csinn_pool_params *params)
{
    ...
    for (int b = 0; b < batch; b++) {
        float *in_ptr = input_data + b * in_h * in_w * in_c;
        ...
        for (int oh = 0; oh < out_h; oh++) {
            ...
            for (; ow < dst_1x8_start_w; ow++) {
                maxpool_border_fp32_nhwc(in_ptr, out_ptr, params, oh, ow, idx_h_start, idx_h_end,
                                         in_w, out_w, in_c);
            }
            for (; ow + 8 <= dst_1x8_end_w; ow += 8) {
                maxpool_w8_fp32_nhwc(in_ptr, out_ptr, params, oh, ow, idx_h_start, idx_h_end, in_w,
                                     out_w, in_c);
            }
            for (; ow < out_w; ow++) {
                maxpool_border_fp32_nhwc(in_ptr, out_ptr, params, oh, ow, idx_h_start, idx_h_end,
