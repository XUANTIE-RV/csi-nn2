...
static void avgpool_w8_fp32_packn(const float *src, float *dst, struct csinn_pool_params *params,
                                  ...
                                  int out_w, enum avgpool_loc_enum loc)
{
    ...
    for (int h = idx_h_start; h < idx_h_end; h++) {
        for (int w = idx_w_start; w < idx_w_end; w++) {
            ...
            _acc0 = vfadd_vv_f32m1(_acc0, vle32_v_f32m1(in_ptr + 0 * stride_w * packn, vl), vl);
            _acc1 = vfadd_vv_f32m1(_acc1, vle32_v_f32m1(in_ptr + 1 * stride_w * packn, vl), vl);
            _acc2 = vfadd_vv_f32m1(_acc2, vle32_v_f32m1(in_ptr + 2 * stride_w * packn, vl), vl);
            _acc3 = vfadd_vv_f32m1(_acc3, vle32_v_f32m1(in_ptr + 3 * stride_w * packn, vl), vl);
            _acc4 = vfadd_vv_f32m1(_acc4, vle32_v_f32m1(in_ptr + 4 * stride_w * packn, vl), vl);
            _acc5 = vfadd_vv_f32m1(_acc5, vle32_v_f32m1(in_ptr + 5 * stride_w * packn, vl), vl);
            _acc6 = vfadd_vv_f32m1(_acc6, vle32_v_f32m1(in_ptr + 6 * stride_w * packn, vl), vl);
            _acc7 = vfadd_vv_f32m1(_acc7, vle32_v_f32m1(in_ptr + 7 * stride_w * packn, vl), vl);
        }
    }

    float *out_ptr = dst + (oh * out_w + ow) * packn;
    vse32_v_f32m1(out_ptr + 0 * packn, vfmul_vf_f32m1(_acc0, ratio, vl), vl);
    vse32_v_f32m1(out_ptr + 1 * packn, vfmul_vf_f32m1(_acc1, ratio, vl), vl);
    vse32_v_f32m1(out_ptr + 2 * packn, vfmul_vf_f32m1(_acc2, ratio, vl), vl);
    vse32_v_f32m1(out_ptr + 3 * packn, vfmul_vf_f32m1(_acc3, ratio, vl), vl);
    vse32_v_f32m1(out_ptr + 4 * packn, vfmul_vf_f32m1(_acc4, ratio, vl), vl);
    vse32_v_f32m1(out_ptr + 5 * packn, vfmul_vf_f32m1(_acc5, ratio, vl), vl);
    vse32_v_f32m1(out_ptr + 6 * packn, vfmul_vf_f32m1(_acc6, ratio, vl), vl);
    vse32_v_f32m1(out_ptr + 7 * packn, vfmul_vf_f32m1(_acc7, ratio, vl), vl);
}

static void avgpool_border_fp32_packn(const float *src, float *dst,
                               ...
                               struct csinn_pool_params *params)
{
    ...
    for (int bc = 0; bc + packn - 1 < batch * in_c; bc += packn) {
        const float *in_ptr = input_data + bc * in_h * in_w;
        ...
        for (; oh < dst_start_h; oh++) {
            ...
            for (; ow < dst_1x8_start_w; ow++) {
                avgpool_border_fp32_packn(in_ptr, out_ptr, params, oh, ow, idx_h_start, idx_h_end,
                                          in_w, out_w, AVGPOOL_LEFT_TOP);
            }
            for (; ow + 7 < dst_1x8_end_w; ow += 8) {
                avgpool_w8_fp32_packn(in_ptr, out_ptr, params, oh, ow, idx_h_start, idx_h_end, in_w,
                                      out_w, AVGPOOL_TOP);
            }
            for (; ow < out_w; ow++) {
                avgpool_border_fp32_packn(in_ptr, out_ptr, params, oh, ow, idx_h_start, idx_h_end,
