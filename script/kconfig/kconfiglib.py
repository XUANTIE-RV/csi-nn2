...
class Kconfig(object):
    ...
    def write_config(self, filename=None, header=None, save_old=True,
        ...
        cmake_file = standard_cmake_config_filename()
        cmake_contents = convert_to_cmake(contents)
        with self._open(cmake_file, "w") as f:
            f.write(cmake_contents)

        header_file = standard_config_header_filename()
        header_contents = convert_to_header(contents)
        with self._open(header_file, "w") as f:
...
def standard_cmake_config_filename():
    """
    Helper for tools. Returns the value of KCONFIG_CONFIG (which specifies the
    .config file to load/save) if it is set, and ".config" otherwise.
    ...
    without having to use this function.
    """
    return os.getenv("KCONFIG_CONFIG", "config.cmake")


def standard_config_header_filename():
    """
    Helper for tools. Returns the value of KCONFIG_CONFIG (which specifies the
    .config file to load/save) if it is set, and ".config" otherwise.
    ...
    without having to use this function.
    """
    return os.getenv("KCONFIG_CONFIG", "mconf_config.h")


def convert_to_cmake(contents):
    ret = ""
    cmake_content = re.split('\n', contents)

    for content in cmake_content:
        if not re.match('#', content):
            if '=y' in content:
                c_key, c_value = re.split('=', content)
                #c_key = c_key.replace("CONFIG_", "")
                c_value = c_value.replace("y", "ON")
                ret += "set({} {})\n".format(c_key, c_value)

    return ret


def convert_to_header(contents):
    ret = ""
    header_content = re.split('\n', contents)

    for content in header_content:
        if re.match('#', content):
            if 'is not set' in content:
                c_key, _ = re.split(' is ', content)
                _, c_key = re.split(' ', c_key)
                ret += "#define {}_DISABLED\n".format(c_key)
